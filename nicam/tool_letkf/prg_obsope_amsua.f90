...
program prg_obsope_amsua
  ...
  REAL(8),ALLOCATABLE,SAVE :: tvsdat_org(:,:,:,:)
  ...
  allocate( tvsdat_org(maxtvsch,maxtvsprof,ninstrument,nslots) )
...
!#############################################################################!
    ...
    do islot = 1, nslots
      do n =  1, ntvsprof(nn,islot)
        ...
        !        write(ADM_LOG_FID,*) 'cloud', n, &
        !          tvslon(n,nn,islot), tvslat(n,nn,islot), &
        !          tvsdat(4,n,nn,islot)-bt_tmp(4,n,nn,islot)
        !      end if
