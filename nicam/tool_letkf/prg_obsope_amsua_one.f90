...
program prg_obsope_amsua
  ...
  INTEGER,PARAMETER :: ninstrument=4
  ...
  write(ADM_LOG_FID,*) "obsfile"
  write(ADM_LOG_FID,*) trim(obsfile)
  FLUSH(ADM_LOG_FID)
...
!  
    ...
    do islot = 1, nslots
      do n =  1, ntvsprof(nn,islot)
        !if( abs(tvsdat(1,n,nn,islot)-bt_tmp(1,n,nn,islot)) > 50.0d0 .or.
        !    abs(tvsdat(2,n,nn,islot)-bt_tmp(2,n,nn,islot)) > 50.0d0 ) then
        !  write(ADM_LOG_FID,*) islot, n, tvslon(n,nn,islot),tvslat(n,nn,islot),&
        !    tvsdat(1,n,nn,islot)-bt_tmp(1,n,nn,islot), &
        !    tvsdat(2,n,nn,islot)-bt_tmp(2,n,nn,islot)
        !else
          do ic=1, ntvsch(nn)
            ichan=tvsch(ic,nn)
            !if(tvslat(n,nn,islot)>=-60.0 .and. lsql(n,nn,islot)==0 ) then
            !  tvsqc(ic,n,nn,islot)=0 
            !end if
            !if(ichan<9) then
              if(tvselev(n,nn,islot)>1000 ) then
                tvsqc(ic,n,nn,islot)=0
              end if
    ...
    !do nn = 1, ninstrument
      ...
      do n = 1, ntvsprof(nn,islot)
        write(ofid) &
           real(tvslon(n,nn,islot),kind=8),                           &
           real(tvslat(n,nn,islot),kind=8),                           &
           real(obsdata_jprb(1:nlev, n, nn, id_pres_nicam, islot),kind=8), &
           real(obsdata_jprb(1:nlev, n, nn, id_temp_nicam, islot),kind=8), &
           real(obsdata_jprb(1:nlev, n, nn, id_qvap_nicam, islot),kind=8), &
           bt_tmp(tvsch(1:ntvsch(nn),nn),n,nn,islot),                 &
           real(tvsdat(tvsch(1,nn):tvsch(ntvsch(nn),nn),n,nn,islot),kind=8)
        i=i+1
      enddo
      close(ofid)
    !enddo
...
!
  ...
  !
        ...
        do n = 1, ntvsprof(nn,islot)
           ...
           write(ofid2,'(38F10.2)') &
           real(obsdata_jprb(1:nlev, n, nn, id_pres_nicam, islot)) 
           write(ofid2,'(38F10.2)') &
           real(obsdata_jprb(1:nlev, n, nn, id_temp_nicam, islot)) 
           write(ofid2,'(38F10.2)') &
           real(obsdata_jprb(1:nlev, n, nn, id_qvap_nicam, islot))
           write(ofid2,'(3F10.2)') &
           bt_tmp(tvsch(1:ntvsch(nn),nn),n,nn,islot)
           write(ofid2,'(3F10.2)') &
