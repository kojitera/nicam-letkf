...
!========================================================================
  ...
  !do ic = 1, ntvsch(nn)
    gall=ntvsprof(isate)
    call Desroziers(isate, ichan, gall,       &
                    lall,       &
                    ndate,      &
                    real(tmpqc0(ichan,1:gall,isate,1)),   &
                    real(tmpqc0(ichan,1:gall,isate,1)),   &
                    tmplon(1:gall,isate,1),     &
                    tmplat(1:gall,isate,1),     &
                    ddd_all(1:gall,ichan,isate),    &
                    zzz_all(1:gall,ichan,isate),    &
                    asiz,       &
                    lngmax,     &
                    myrank,     &
                    nprocs,     &
...
!!! Dstat with Hollingsworth (1986)     <ddT> = R + HBHT
  ...
  do idate=1,ndate
    ...
    do g = 1,nsamp
    do l = 1,nsamp
      call calc_length_deg(dlon(g), dlon(l), dlat(g), dlat(l), length(l), angle(l)) !! length [m], angle [deg]
      if(g.eq.1 .and. length(l)*0.001 < 1000.0 ) then
        write(*,'(i8,5F12.5)') l, dlon(g), dlon(l), dlat(g), dlat(l), length(l)*0.001
      end if
    end do
    do l = 1,nsamp
      if( g == l ) then
        ...
        length(l) = length(l) * 0.001d0 !! [m] --> [km]
        ilng = int( undef )
        if ( length(l).lt.lngmax+asiz*0.5d0 ) then      
          ilng = int( (length(l)-asiz*0.5d0) / asiz ) + 1
          if( length(l).lt.0.00001 ) ilng = 0
          acnt(ilng) = acnt(ilng) + 1.0
          ades(ilng) = ades(ilng) + d(g)*z(l) !  R=<(y-Hx^a)(y-Hx^f)T>
          abes(ilng) = abes(ilng) + z(g)*z(l) !  <(y-Hx^f)(y-Hx^f)T>
          ahol(ilng) = ahol(ilng) + d(g)*d(l) !  <ddT> = R + HBHT
...
!!!    print '(4f)', length, acnt(ilng), ades(ilng), ahol(ilng)
    write(fnum,'(2i6,f8.1,i15,6f15.10)') inst, ic, length(l), int(acnt(ilng)), &
      ades(ilng), ades(ilng)/ades(0), &
      !ades(ilng), ades(ilng)/ades(0)/sqrt(1.0d0+(ades(ilng)/ades(0))**2), &
...
!    real(8), parameter :: Rpo = 6334834.0d0, Req = 6377937.0d0
    ...
    cosd = SIN(ilat)*SIN(jlat) + COS(ilat)*COS(jlat)*COS(jlon-ilon)

    cosd = MIN( 1.0,cosd)
    cosd = MAX(-1.0,cosd)

    length = ACOS( cosd ) * RRR
