...
program dstat
  ...
  character(256) fname_10
  character(256) fname_11
  ...
  integer :: isate ! for MPI
  integer :: ichan ! for MPI
...
!========================================================================
  ...
  gues_fname='AA18_gues.dat'
  anal_fname='AA18_anal.dat'
  ...
  !isate=3
  write(*,*) 'myrank=', myrank
  write(*,*) 'sate', tvsname(isate)
  write(fname_10,'(a,i5.5,a)') trim(tvsname(isate)), ichan, '_R.txt'
  write(fname_11,'(a,i5.5,a)') trim(tvsname(isate)), ichan, '_.txt'

  write(*,*) 'fname_10', trim(fname_10)
  write(*,*) 'fname_11', trim(fname_11)

  open(10,file=trim(fname_10),action='write')
  !open(11,file=trim(fname_11),action='write')
  !do nn = 1, ninstrument
  !do ic = 1, ntvsch(nn)
    gall=ntvsprof(isate)
    call Desroziers(isate, ntvsch(isate), gall,                   &
                    lall,                                         &
                    ndate,                                        &
                    real(tmpqc0(1:ntvsch(isate),1:gall,isate,1)), &
                    real(tmpqc0(1:ntvsch(isate),1:gall,isate,1)), &
                    tmplon(1:gall,isate,1),                       &
                    tmplat(1:gall,isate,1),                       &
                    ddd_all(1:gall,1:ntvsch(isate),isate),        &
                    zzz_all(1:gall,1:ntvsch(isate),isate),        &
                    asiz,                                         &
                    lngmax,                                       &
                    myrank,                                       &
                    nprocs,                                       &
...
!=========================================================
subroutine Desroziers(inst,nc,gall,lall,ndate,msk,msk_rgn,ico_lon,ico_lat,ddd,zzz,asiz,lngmax,myrank,nprocs,rtimer00,fnum,mnum,ccase,csurf)
!=========================================================
...
!!! Dstat with Hollingsworth (1986)     <ddT> = R + HBHT
  ...
  real(4), intent(in)  :: ddd(gall,nc,ndate), zzz(gall,nc,ndate), msk(nc,gall,ndate), msk_rgn(nc,gall,lall)
  real(4), intent(in)  :: rtimer00
  integer, intent(in)  :: inst, nc
  character(3), intent(in) :: ccase, csurf
  !===> global variables
  integer :: g, l, idate, icon, nmax, nsamp, ilng, nlng, ic, kc
  real(4) :: rtimer, tmpmsk(nc,gall)
  real(4), allocatable :: d(:,:), z(:,:)  
  real(8), allocatable :: dlon(:,:), dlat(:,:), acnt(:), ades(:), ahol(:), afnc(:)
  real(8), allocatable :: abes(:)
  real(4), parameter :: undef = -9999.
  real(8) :: RRR, RHBHT
  real(8), allocatable :: length(:), angle(:)
  real(8), allocatable :: R_des(:,:)

  !===> cnt
  nlng  = nc
  !nlng  = int( lngmax / asiz )  
  allocate ( acnt(0:nlng), ades(0:nlng), ahol(0:nlng), afnc(0:nlng), abes(0:nlng) )
  allocate ( R_des(nc,nc) )
  ...
  do g=1,gall
  do ic=1,nc
    tmpmsk(nc,g) = maxval( msk(ic,g,1:ndate) )
    if( tmpmsk(nc,g)>0.5 ) nmax = nmax + 1
  end do
  end do
  allocate ( d(nmax,nc), z(nmax,nc), dlon(nmax,nc), dlat(nmax,nc) )
  allocate ( length(nmax), angle(nmax) )
  ...
  do idate=1,ndate
    ...
    !do ic=1,nc
    do g=1,gall
      !if ( msk(g,ic,idate)>0.5 .and. msk_rgn(g,ic,idate)>0.5 ) then
      if ( minval(msk(:,g,idate))==1 ) then
        nsamp = nsamp + 1
        d(nsamp,:)    = ddd(g,:,idate)
        z(nsamp,:)    = zzz(g,:,idate)
        !dlon(nsamp,ic) = ico_lon(g,1)
        !dlat(nsamp,ic) = ico_lat(g,1) 
      end if
    end do
    !end do
  write(*,*) 'nsamp   =', nsamp
    ...
    do kc = 1, nc
      do g = 1, nsamp
        R_des(kc,ic)=R_des(kc,ic) + d(g,ic)*z(g,kc)
      end do
    end do 
    end do 
  ...
  R_des(:,:)=R_des(:,:)/dble(nsamp)

!  write(*,*) "Before Normalize"
!  do ic = 1, nc
!    write(fnum,'(10F8.4)') (R_des(kc,ic),kc=1,nc)
!  end do

  do ic = 1, nc
  do kc = 1, nc
    R_des(kc,ic)=R_des(kc,ic)/(sqrt(R_des(ic,ic))*sqrt(R_des(kc,kc)))
  end do
  end do

!  write(*,*) "After Normalize"
!  do ic = 1, nc
!    write(fnum,'(10F8.4)') (R_des(kc,ic),kc=1,nc)
!  end do

  R_des(:,:)=0.5d0*(R_des(:,:)+transpose(R_des(:,:)))
...
!  end do
  do ic = 1, nc
  do kc = 1, nc
    write(fnum,'(2i6,F12.7)') ic, kc, R_des(kc,ic)
  end do
  end do
...
!    length = dacos ( dsin(ilat)*dsin(jlat) + dcos(jlat)*dcos(ilat)*dcos(ilon-jlon) ) * RRR
    ...
    A = Rpo/(1.d0 - e*tmp_sin)**(1.5d0)
    B = Req/(1.d0 - e*tmp_sin)**(0.5d0)
    length = dsqrt( (A*dP)**2 + (B*tmp_cos*dR)**2)    
