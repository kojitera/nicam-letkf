MODULE mod_read_history
  ...
  CHARACTER(LEN=FIO_HLONG), PRIVATE :: indir(flim)        = ''
...
SUBROUTINE read_history_init
  ...
  NAMELIST / readhistory_cnf /  &
       rgnmngfname,            &
       indir

  OPEN(90,file='obsope.cnf')
  READ(90,NML=readhistory_cnf)
  CLOSE(90)
...
END SUBROUTINE read_history_init
!------------------------------------------------------------------------------
SUBROUTINE read_history(imem)
  USE mod_adm
  USE mod_obsope_common
  USE mod_fio, ONLY : &
       FIO_HSHORT,       &
  ...
  IF( imem > 0 ) THEN
    infile_header(1)=trim(indir(1))//'/'//trim(cimem)//'/history'
  ELSE IF ( imem == -1 ) THEN
    infile_header(1)=trim(indir(1))//'/history_anal_me'
    istep=1
  ELSE IF ( imem == -2 ) THEN
  ...
  DO p = pstr, pend
    ...
    ALLOCATE( data8allrgn(ADM_gall*ADM_vlayer*MNG_prc_rnum(p)) )
    DO nv = 1, nv3d
    ...
    ALLOCATE( data8allrgn(ADM_gall*MNG_prc_rnum(p)) )
