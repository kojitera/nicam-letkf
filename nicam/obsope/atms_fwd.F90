SUBROUTINE ATMS_fwd(nlevels, nprof, coef_filename, tmp_p, tmp_t, tmp_q, tmp_t2m, tmp_q2m, &
...
#include "rttov_skipcommentline.interface"
  ...
  Real(Kind=8) :: bt_out(22,nprof)
  Real(Kind=8) :: tran_out(nlevels,22,nprof)
...
!  CLOSE(iup)
  ...
  DO iprof = 1, nprof
    ...
    bt_out(1:22,iprof)=radiance%bt(1+joff:nchannels+joff)
    DO ilev = 1, nlevels
       tran_out(ilev,1:22,iprof)=transmission % tau_levels(ilev,1+joff:nchannels+joff) 
    ENDDO
