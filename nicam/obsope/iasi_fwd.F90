SUBROUTINE IASI_fwd(nlevels, nprof, coef_filename, &
                    datetime,  &
                    tmp_co2,   &
                    tmp_p,     &
                    tmp_t,     &
...
#include "rttov_skipcommentline.interface"
  ...
  Integer(Kind=jpim) :: datetime(1:3, nprof) 
  Real(Kind=jprb) :: tmp_co2(nlevels, nprof)
  ...
  Real(Kind=8) :: bt_out(nchannels,nprof)
  Real(Kind=8) :: tran_out(nlevels,nchannels,nprof)
...
!  ENDDO
  ...
  CALL rttov_read_coefs(errorstatus, coefs, opts, form_coef='unformatted', file_coef=coef_filename)
  IF (errorstatus /= errorstatus_success) THEN
    WRITE(*,*) 'fatal error reading coefficients'
    CALL rttov_exit(errorstatus)
  ENDIF
  tmp_time(2)=MPI_WTIME()
  time_rttov(1)=time_rttov(1)+(tmp_time(2)-tmp_time(1))

  !WRITE(*,*) 'nchannels= ', nchannels
  !WRITE(*,*) 'fmv_chn  = ', coefs % coef % fmv_chn
  ...
  DO iprof = 1, nprof
    profiles(iprof)%date(:)=datetime(:,iprof)
    profiles(iprof)%co2(:)=tmp_co2(:,iprof)
...
!  CLOSE(iup)
  ...
  time_rttov(2)=time_rttov(2)+(tmp_time(4)-tmp_time(3))
  ...
  DO iprof = 1, nprof
    ...
    bt_out(1:nchannels,iprof)=radiance%bt(1+joff:nchannels+joff)
    DO ilev = 1, nlevels
       tran_out(ilev,1:nchannels,iprof)=transmission % tau_levels(ilev,1+joff:nchannels+joff)
    ENDDO
  END DO
