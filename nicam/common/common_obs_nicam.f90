...
!$USE OMP_LIB
  ...
  INTEGER,PARAMETER :: id_p_obs=3130
  INTEGER,PARAMETER :: id_qv_obs=3330
  INTEGER,PARAMETER :: id_qc_obs=3430
...
SUBROUTINE Trans_XtoY(elm,ri,rj,rk,v3d,v2d,p_full,yobs)
  ...
  !CASE(id_p_obs)  ! P
  !  CALL itpl_3d(v3d(:,:,:,iv3d_p),ri,rj,rk,yobs)
  CASE(id_qv_obs)  ! Qv
    CALL itpl_3d(v3d(:,:,:,iv3d_qv),ri,rj,rk,yobs)
  CASE(id_qc_obs)  ! Qc
    CALL itpl_3d(v3d(:,:,:,iv3d_qc),ri,rj,rk,yobs)
  CASE(id_ps_obs) ! PS
...
SUBROUTINE monit_dep(nn,elm,dep,qc)
  ...
  REAL(r_size) :: rmse_u,rmse_v,rmse_t,rmse_qv,rmse_ps,rmse_rh
  REAL(r_size) :: bias_u,bias_v,bias_t,bias_qv,bias_ps,bias_rh
  INTEGER :: n,iu,iv,it,iqv,ips,irh
  ...
  DO n=1,nn
    ...
    CASE(id_t_obs)
      ...
      it = it + 1
    CASE(id_qv_obs)
      rmse_qv = rmse_qv + dep(n)**2
      bias_qv = bias_qv + dep(n)
      iqv = iqv + 1
    CASE(id_ps_obs)
  ...
  IF(iqv == 0) THEN
    ...
    bias_qv = undef
  ELSE
    rmse_qv = SQRT(rmse_qv / REAL(iqv,r_size))
    bias_qv = bias_qv / REAL(iqv,r_size)
  END IF
  IF(ips == 0) THEN
    rmse_ps = undef
    bias_ps = undef
  ...
  WRITE(ADM_LOG_FID,'(A)') '== OBSERVATIONAL DEPARTURE ============================================='
  WRITE(ADM_LOG_FID,'(6A12)') 'U','V','T','Q','PS','RH'
  WRITE(ADM_LOG_FID,'(6ES12.3)') bias_u,bias_v,bias_t,bias_qv,bias_ps,bias_rh
  WRITE(ADM_LOG_FID,'(6ES12.3)') rmse_u,rmse_v,rmse_t,rmse_qv,rmse_ps,rmse_rh
  WRITE(ADM_LOG_FID,'(A)') '== NUMBER OF OBSERVATIONS TO BE ASSIMILATED ============================'
  WRITE(ADM_LOG_FID,'(6A12)') 'U','V','T','Q','PS','RH'
  WRITE(ADM_LOG_FID,'(6I12)') iu,iv,it,iqv,ips,irh
  WRITE(ADM_LOG_FID,'(A)') '========================================================================'
...
SUBROUTINE get_nobs(cfile,nrec,nn)
  ...
  IF(ex) THEN
    ...
    DO
      ...
      nn = nn + 1
    END DO
    WRITE(ADM_LOG_FID,'(I10,A)') nn,' OBSERVATIONS INPUT'
    WRITE(ADM_LOG_FID,'(A12,I10)') '          U:',iu
    WRITE(ADM_LOG_FID,'(A12,I10)') '          V:',iv
    WRITE(ADM_LOG_FID,'(A12,I10)') '          T:',it
    WRITE(ADM_LOG_FID,'(A12,I10)') '          Q:',iqv
    WRITE(ADM_LOG_FID,'(A12,I10)') '         RH:',irh
    WRITE(ADM_LOG_FID,'(A12,I10)') '         Ps:',ips
    CLOSE(iunit)
  ELSE
    WRITE(ADM_LOG_FID,'(2A)') cfile,' does not exist -- skipped'
  END IF
  DEALLOCATE(wk)
...
SUBROUTINE read_obs2(cfile,nn,elem,rlon,rlat,rlev,odat,oerr,ohx,oqc,typ)
  IMPLICIT NONE
  CHARACTER(*),INTENT(IN) :: cfile
  INTEGER,INTENT(IN) :: nn
  REAL(r_size),INTENT(OUT) :: elem(nn) ! element number
